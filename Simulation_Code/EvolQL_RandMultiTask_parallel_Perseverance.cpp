...
const double phi_upper = 10.0;
const double phi_lower = 0.0;
const double mut_phi = 0.1;
...
const string init_phi = "R";
const double fixed_phi = 0.0;
...
// ref: https://code-database.com/knowledges/122 
...
class Agent{
    ...
    public:
        ...
        void set_Genes(double alp, double aln, double beta, double phival) {ap = alp; an = aln; bt = beta; phi = phival;}
        void show_Genes() {cout << "ap: " << ap << "; an: " << an << "; bt: " << bt << "; phi: " << phi << endl;}
        double return_ap() {return ap;}
        double return_an() {return an;}
        double return_bt() {return bt;}
        double return_phi() {return phi;}
...
void Agent::select_Option(int task_i, int trial_i, vector<double> &payoff_structure) {
    ...
    if (trial_i == 0) { // in the first trial

        Pvalue_list[task_i][trial_i] = 1/(1 + exp(- bt * (Qvalue_list[trial_i][1]-Qvalue_list[trial_i][0])));
        //cout << "P value at 1st trial " << Pvalue_list[task_i][trial_i] << endl; 

    }
    else {

        // if the choice in the previous trial = 1, add -phi for the p value (choice1)
        if (Choice_list[task_i][trial_i -1] == 1) {

            Pvalue_list[task_i][trial_i] = 1/(1 + exp(- bt * (Qvalue_list[trial_i][1]-Qvalue_list[trial_i][0]) - phi));

        } 
        // if the choice in the previous trial = 0, add +phi for the p value (choice1)
        else if (Choice_list[task_i][trial_i -1] == 0) {

            Pvalue_list[task_i][trial_i] = 1/(1 + exp(- bt * (Qvalue_list[trial_i][1]-Qvalue_list[trial_i][0]) + phi));

        }
        else {
            cout << "Stop. Error in Choice_list[task_i][trial_i -1]. See the calculation of Pvalue." << endl;
            terminate();
        }
    ...
    //cout << "p value is : " << Pvalue_list[task_i][trial_i] << endl;
    ...
    //cout << "Choice 1 or 0: " << Choice_list[task_i][trial_i] << endl;
...
class saveData {
    ...
    public:
        ...
        void recordData_trial(Agent agent, int sim_i, int gene_i, int pop_i, int task_i, int trial_i);
        void recordData_nat(Agent agent, int sim_i, int gene_i, int pop_i, int task_i);
        void recordData_gene(Agent agent, int sim_i, int gene_i, int pop_i);
        
        // functions to write output to files
        void writeParameterInfo();
        void writeTaskInfo(vector<int> &sampled_tasks);
        void writeElapsedTime(int sim_i, int gene_i, int check_i);
...
saveData::saveData()
{
    ...
    // initialize output
    output_trial.assign(PopSize*Task_N*Trial, vector<double>(15, 0.0)); // 15 columns
    //output_nat.assign(PopSize, vector<double>(17, 0.0)); // full output ver. 17 columns
    output_nat.assign(PopSize*Task_N, vector<double>(8, 0.0)); // simple output ver. 8 columns
    output_gene.assign(10, 0.0); // 10 columns

}

// function to create directory to make files
void saveData::create_dir(string filename) {
    ...
    */
    ...
    task_path_name += path_name; task_path_name += "task_info/";
    if (mkdir(task_path_name.c_str(), 0777) == 0)
        cout << "---- create the directory ..." << endl << task_path_name.c_str() << endl;

    // Copy this .cpp file to the directory
    system_order_st_cp = "cp EvolQL_RandMultiTask_parallel_Perseverance.cpp ";
...
void saveData::create_file() {
    ...
    stringstream rep;
    rep << "sim" << setw(3) << setfill('0') << sim_i; 
    ...
    for (int file_trial_i = 0; file_trial_i < 10; ++file_trial_i) {
        ...
        if (!ofs1) {
            ...
            ofs1 << "sim,generation,agent,task,trial,ap,an,bt,Q1,Q0,p,choice,get_payoff,delta" << endl;
        }
    ...
    */
    ...
    if (!ofs1) {
        ...
        cout << "succeed! created " << file_name_trial << endl;
        // 15 columns, write the header names
        ofs1 << "sim,generation,agent,task,trial,ap,an,bt,phi,Q1,Q0,p,choice,get_payoff,delta" << endl;
    }
    ...
    if (!ofs2) {
        ...
        // full output ver. (17 columns), write the header names
        //ofs2 << "sim,generation,agent,task,ancestor,mean_payoff,mean_p,fitness,rate_choice1,rate0to0,rate0to1,rate1to0,rate1to1,ap,an,bt,phi" << endl;
        
        // short output ver. (8 columns), write the header names
        // cf. BasicEvolQL_parallel.cpp: This short ver mainly omit 3 columns: sim, generation, and agent
        // because they make file size too large and redundant info.
        ofs2 << "task,mean_payoff,mean_p,rate_choice1,ap,an,bt,phi" << endl; 
    }
    ...
    if (!ofs3) {
        ...
        // 10 columns, write the header names
        ofs3 << "sim,generation,mean_ap,mean_an,mean_bt,mean_phi,sd_ap,sd_an,sd_bt,sd_phi" << endl;
    }

    file_name_task += task_path_name; file_name_task += rep.str(); file_name_task += "_task.csv";
    ofstream ofs_task(file_name_task);
    if (!ofs_task) {
        cout << "cannot open file!. Check file_name ..." << endl << file_name_task << endl;
    }
    else {
        cout << "succeed! created " << file_name_task << endl;
        // header names
        ofs_task << "sim_i,task_i,m1,sd1,m2,sd2,task_number" << endl;
    }
...
void saveData::writeParameterInfo() {
    ...
    fout_params << "Script Name : " << script_name << endl << "Run_condition : " << Run_condition << endl << "Number of Replication : " << Replication << endl << \
     ...
     "Risk-seeking task_N : " << Riskseek_N << endl << \
     "mutation_ap : " << mut_ap << endl << "mutation_an : " << mut_an << endl << "mutation_bt : " << mut_bt << endl << "mutation phi : " << mut_phi << endl << \
     "Sim_N : " << SimN << endl << "Task Group_N : " << Taskgroup_N << endl << \
     "type_op : " << "Normal" << endl << \
     "init_alpha_positive : " << init_ap << endl << "init_alpha_negative : " << init_an << endl << "init_inverse_temp : " << init_bt << endl << \
     "init_phi : " << init_phi << endl << \
     "limit_inverse_temp : " << bt_upper << endl << "phi_lower : " << phi_lower << endl << "phi_upper : " << phi_upper << endl << \
...
void saveData::writeTaskInfo(vector<int> &sampled_tasks) {

    ofstream fout_task(file_name_task, ios::app);

    for (int task_i = 0; task_i < Task_N; ++task_i) {

        fout_task << sim_i << "," << task_i << "," << payoff_norm_mean1_list[task_i] << "," << payoff_norm_sigma1_list[task_i] << "," << \
        payoff_norm_mean2_list[task_i] << "," << payoff_norm_sigma2_list[task_i] << "," << sampled_tasks[task_i] << endl;
...
void saveData::writeElapsedTime(int sim_i, int gene_i, int check_i) {
    ...
    ofstream fout_params(file_name_params, ios::app);
    fout_params << "sim_i : " << sim_i << ", gene_i : " << gene_i << \
...
void saveData::recordData_trial(Agent agent, int sim_i, int gene_i, int pop_i, int task_i, int trial_i) {

    double output[15] = {(double)sim_i, (double)gene_i, (double)agent.return_ID(), (double)task_i, (double)trial_i, \
                        agent.return_ap(), agent.return_an(), agent.return_bt(), agent.return_phi(), \
    ...
    for (int data_i = 0; data_i < 15; ++data_i) {
...
void saveData::recordData_nat(Agent agent, int sim_i, int gene_i, int pop_i, int task_i) {

    /*
    // full output version (17 columns)
    double output[17] = {(double)sim_i, (double)gene_i, (double)agent.return_ID(), (double)task_i, (double)agent.return_Offspring(), agent.return_MeanPayoff(task_i), \
    ...
    for (int data_i = 0; data_i < 17; ++data_i) {
        output_nat[pop_i][data_i] = output[data_i];
    }
    */

    // short output version (8 colums)
    double output[8] = {(double)task_i, agent.return_MeanPayoff(task_i),\
                        (double)agent.return_MeanP(task_i), agent.return_rateChoice1(task_i), \
                        agent.return_ap(), agent.return_an(), agent.return_bt(), agent.return_phi()};
...
void saveData::recordData_gene(Agent agent, int sim_i, int gene_i, int pop_i) {
    ...
    phi_list[pop_i] = agent.return_phi();
    //cout << "Name : " << pop_i << " ap : " << agent.return_ap() << " an : " << agent.return_an() << " bt : " << agent.return_bt() << " phi : " << agent.return_phi() << endl;
    ...
    if (pop_i == PopSize - 1) {
        ...
        */
        ...
        double mean_ap, mean_an, mean_bt, mean_phi;
        double sum_ap = 0.0; double sum_an = 0.0; double sum_bt = 0.0; double sum_phi = 0.0;

        for (int i = 0; i < PopSize; ++i) {

            //cout << "Name : " << i << " ap : " << ap_list[i] << " an : " << an_list[i] << " bt : " << bt_list[i] << "phi : " << phi_list[i] << endl;
            sum_ap += ap_list[i]; 
            sum_an += an_list[i]; 
            sum_bt += bt_list[i];
            sum_phi += phi_list[i];
        ...
        mean_ap = sum_ap/PopSize; mean_an = sum_an/PopSize; mean_bt = sum_bt/PopSize; mean_phi = sum_phi/PopSize;
        //cout << "mean_ap: " << mean_ap << ", mean_an: " << mean_an << ", mean_bt: " << mean_bt << ", mean_phi: " << mean_phi << endl;

        // calculate SD of parameters
        double sd_ap, sd_an, sd_bt, sd_phi;
        double var_ap = 0.0; double var_an = 0.0; double var_bt = 0.0; double var_phi = 0.0;
        ...
        for (int i = 0; i < PopSize; ++i) {
            ...
            var_phi += (phi_list[i] - mean_phi) * (phi_list[i] - mean_phi);

        }
        
        sd_ap = sqrt(var_ap/PopSize); sd_an = sqrt(var_an/PopSize); sd_bt = sqrt(var_bt/PopSize); sd_phi = sqrt(var_phi/PopSize);
        //cout << "sd_ap: " << sd_ap << ", sd_an: " << sd_an << ", sd_bt: " << sd_bt << ", sd_phi: " << sd_phi << endl;

        double output[10] = {(double)sim_i, (double)gene_i, mean_ap, mean_an, mean_bt, mean_phi, sd_ap, sd_an, sd_bt, sd_phi};
...
void saveData::writeCSV_trial(int gene_i) {
    ...
    for (size_t data_i = 0; data_i < (PopSize*Trial*Task_N); ++data_i) {
        ...
        for (size_t data_j = 0; data_j < 15; ++data_j) {
...
void saveData::writeCSV_nat() {
    ...
    for (size_t data_i = 0; data_i < PopSize; ++data_i) {
        ...
        for (size_t data_j = 0; data_j < 17; ++data_j) {
    ...
    */
    ...
    for (size_t data_i = 0; data_i < (PopSize*Task_N); ++data_i) {
        ...
        for (size_t data_j = 0; data_j < 8; ++data_j) {
...
void saveData::writeCSV_gene() {
    ...
    for (size_t data_i = 0; data_i < 10; ++data_i) {
...
void saveData::gzipCSV() {
    ...
    ofstream fout_params(file_name_params, ios::app);
    fout_params << "sim_i : " << sim_i << ", gzip _nat.csv, order: " << system_order_st_gzip << endl;
    fout_params << "sim_i : " << sim_i << ", end time : " << now_time_str << ", elapsed time (since the last generation) : " << elapsed << " sec" << endl;
    ...
    //ofstream fout_params(file_name_params, ios::app);
    fout_params << "sim_i : " << sim_i << ", gzip _trial.csv, order: " << system_order_st_gzip << endl;
    fout_params << "sim_i : " << sim_i << ", end time : " << now_time_str << ", elapsed time (gzip _trial.csv) : " << elapsed << " sec" << endl;
...
int main(int argc, char *argv[]){
    ...
    Trial = atoi(argv[4]);
    Task_N = atoi(argv[5]);
    Riskseek_N = atoi(argv[6]);
    Taskgroup_N = atoi(argv[7]);

    save_path_name = argv[8];

    // ------- read taskgroup.csv & set task structure ------ //
    
    string taskgroup_path = save_path_name + "_taskgroup.csv";

    vector<vector<string> > taskgroup_data = csv2vector(taskgroup_path, 1);

    vector<int> task_number_vec;
    for (int row_i = 0; row_i < taskgroup_data.size(); ++row_i){
        task_number_vec.push_back(stoi(taskgroup_data[row_i][0]));
    }

    // check whether equal to Taskgroup_N
    if (task_number_vec.size() == Taskgroup_N) {
        //cout << "proper task group set." << endl;
    } else {
        cout << "Stop. task setting is improper." << endl;
        terminate(); // terminate programs
    }

    // former half = risk_seeking tasks, latter half → risk_averse tasks
    vector<int> Riskseeking_tasks_vec;
    vector<int> Riskaverse_tasks_vec;

    for (int task_i = 0; task_i < (Taskgroup_N/2); ++task_i) {

        Riskseeking_tasks_vec.push_back(task_number_vec[task_i]);
        Riskaverse_tasks_vec.push_back(task_number_vec[task_i + (Taskgroup_N/2)]);

    }

    // for randomly sampling tasks, shuffle vectors
    random_device rand_dev;
    mt19937 rand_engine(rand_dev());
    shuffle(begin(Riskseeking_tasks_vec), end(Riskseeking_tasks_vec), rand_engine);
    shuffle(begin(Riskaverse_tasks_vec), end(Riskaverse_tasks_vec), rand_engine);    

    /*
    // show risk-seeking & risk-averse tasks
    cout << "risk-seeking: " << endl;
    for (int i = 0; i < Riskseeking_tasks_vec.size(); ++i) {
        cout << Riskseeking_tasks_vec[i] << ", ";
    }
    cout << endl;

    cout << "risk-averse: " << endl;
    for (int i = 0; i < Riskaverse_tasks_vec.size(); ++i) {
        cout << Riskaverse_tasks_vec[i] << ", ";
    }
    cout << endl;
    */

    // draw task structure & set to payoff vectors
    vector<int> sampled_tasks;
    
    for (int task_i = 0; task_i < Riskseek_N; ++task_i) {
        sampled_tasks.push_back(Riskseeking_tasks_vec[task_i]); // draw risk-seeking tasks
    }

    for (int task_i = 0; task_i < (Task_N - Riskseek_N); ++task_i) {
        sampled_tasks.push_back(Riskaverse_tasks_vec[task_i]); // draw risk-averse tasks
    }

    /*
    cout << "show sampled tasks: " << endl;
    for (int i = 0; i < sampled_tasks.size(); i++){
        cout << sampled_tasks[i] << ", ";
    }
    cout << endl;
    */
    ...
    for (int task_i = 0; task_i < Task_N; ++task_i) {
        ...
        payoff_norm_mean1_list.push_back(stof(taskgroup_data[sampled_tasks[task_i]][1]));
        payoff_norm_sigma1_list.push_back(stof(taskgroup_data[sampled_tasks[task_i]][2]));
        payoff_norm_mean2_list.push_back(stof(taskgroup_data[sampled_tasks[task_i]][3]));
        payoff_norm_sigma2_list.push_back(stof(taskgroup_data[sampled_tasks[task_i]][4]));

    }

    /*
    cout << "task structure: " << endl;
    for (int i = 0; i < Task_N; i++) {

        cout << "N(" << payoff_norm_mean1_list[i] << ", " << payoff_norm_sigma1_list[i] << "), N(" << \
    ...
    */
    ...
    saveData savedat;
    savedat.create_dir(save_path_name);
    savedat.create_file();
    savedat.writeParameterInfo();
    savedat.writeTaskInfo(sampled_tasks);
    ...
    uniform_real_distribution<double> phi_dist(phi_lower, phi_upper);
    ...
    for (int pop_i = 0; pop_i < PopSize; ++pop_i){
        ...
        if (init_phi == "R") {
            phi_i = phi_dist(get_rand_mt_1);
        }
        else { // "Fixed"
            phi_i = fixed_phi;
        }
        
        nAgent[pop_i].set_Genes(ap_i, an_i, bt_i, phi_i);
        //cout << pop_i << ": ";
    ...
    for (int gene_i = 0; gene_i < Generation; ++gene_i) {
            ...
            for (int task_i = 0; task_i < Task_N; ++task_i) {
                ...
                for (int trial_i = 0; trial_i < Trial; ++trial_i) {
                    ...
                    if ((gene_i == 0)||(gene_i == Generation-1)) {
                        savedat.recordData_trial(nAgent[pop_i], sim_i, gene_i, pop_i, task_i, trial_i); // * WARNING * This line should be usually commented out because we dont need by-trial csv
                    }
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) 
            savedat.recordData_gene(nAgent[pop_i], sim_i, gene_i, pop_i);
        ...
        */
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            for (int task_i = 0; task_i < Task_N; ++task_i) {
                ...
                savedat.recordData_nat(nAgent[pop_i], sim_i, gene_i, pop_i, task_i);
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            new_phi[pop_i] = nAgent[vecOffspring[pop_i]].return_phi();
            //cout << "pop:" << vecOffspring[pop_i] << "; ap: " << new_ap[pop_i] << "; an: " << new_an[pop_i] << \
            "; bt: " << new_bt[pop_i] << "; phi: " << new_phi[pop_i] << endl;
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            normal_distribution<double> nrml_dist_phi(new_phi[pop_i], mut_phi);
            ...
            if (init_phi == "R") {
                ...
                do {
                    next_phi_i = nrml_dist_phi(get_rand_mt_3);
                    //cout << "next phi : " << next_phi_i << endl;
                } while((next_phi_i < phi_lower)||(next_phi_i > phi_upper));

            }
            else {
                
                next_phi_i = fixed_phi;
                
            }
            
            // set new genes to a new agent (offspring)
            nAgent[pop_i].set_Genes(next_ap_i, next_an_i, next_bt_i, next_phi_i);
        }
        ...
        */
        ...
        for (int check_i = 0; check_i < 10; ++check_i) {
            ...
            if (gene_bool == 1) { // if it is 10% of generations, let's record the time
                ...
                savedat.writeElapsedTime(sim_i, gene_i, check_i);
    ...
    // gzip *_trial.csv, *_nat.csv via linux command
    savedat.gzipCSV();
