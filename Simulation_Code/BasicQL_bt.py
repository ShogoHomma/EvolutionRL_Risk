...
def assignGenes(agent, sim_info):
    ...
    ap_list = np.linspace(start=0.1, stop=1.0, num=ap_num, dtype='float')
    an_list = np.linspace(start=0.1, stop=1.0, num=an_num, dtype='float')
    bt_list = np.linspace(start=0.05, stop=0.5, num=bt_num, dtype='float')
    ...
    if len(product_gene) == Population:
        ...
        for i,j in enumerate(product_gene):
            ...
            agent[i].setGenes(round(j[0], 4), round(j[1], 4), round(j[2], 4))
