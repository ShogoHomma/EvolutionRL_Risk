...
const double mut_alpha = 0.01;
const double mut_bt = 0.005;
const double bt_upper = 0.5;

const string init_alpha = "R"; // {"R" : Random, "F" : Fixed}
const double fixed_alpha = 0.5; // if init_alpha = "F", fixed_alpha is assigned to all agents. 
...
class Agent{
    ...
    public:
        ...
        void set_Genes(double alp, double beta) {alpha = alp; bt = beta; }
        void show_Genes() {cout << "alpha: " << alpha << "; bt: " << bt << endl;}
        double return_alpha() {return alpha;}
...
void Agent::qLearning(int task_i, int trial_i) {
    ...
    */
    ...
    // update Q value
    Qvalue_list[trial_i + 1][Choice_list[task_i][trial_i]] = Qvalue_list[trial_i][Choice_list[task_i][trial_i]] + alpha * delta; // update for chosen option 
    Qvalue_list[trial_i + 1][1 - Choice_list[task_i][trial_i]] = Qvalue_list[trial_i][1 - Choice_list[task_i][trial_i]]; // update for non-chosen option 
...
saveData::saveData()
{
    ...
    // initialize output
    output_trial.assign(PopSize*Task_N*Trial, vector<double>(13, 0.0)); // 13 columns
    //output_nat.assign(PopSize, vector<double>(15, 0.0)); // full output ver. 15 columns
    output_nat.assign(PopSize*Task_N, vector<double>(6, 0.0)); // simple output ver. 6 columns
    output_gene.assign(6, 0.0); // 6 columns
...
void saveData::create_dir(string filename) {
    ...
    folder_name += "_alpha"; folder_name += init_alpha; 
    ...
    */
    ...
    system_order_st_cp = "cp EvolQL_RandMultiTask_parallel_singleLR.cpp ";
...
void saveData::create_file() {
    ...
    for (int file_trial_i = 0; file_trial_i < 10; ++file_trial_i) {
        ...
        if (!ofs1) {
            ...
            ofs1 << "sim,generation,agent,task,trial,alpha,bt,Q1,Q0,p,choice,get_payoff,delta" << endl;
        }
    ...
    */
    ...
    if (!ofs2) {
        ...
        // full output ver. (15 columns), write the header names
        //ofs2 << "sim,generation,agent,task,ancestor,mean_payoff,mean_p,fitness,rate_choice1,rate0to0,rate0to1,rate1to0,rate1to1,alpha,bt" << endl;
        
        // short output ver. (6 columns), write the header names
        ...
        ofs2 << "task,mean_payoff,mean_p,rate_choice1,alpha,bt" << endl; 
    }
    ...
    if (!ofs3) {
        ...
        ofs3 << "sim,generation,mean_alpha,mean_bt,sd_alpha,sd_bt" << endl;
    }
...
void saveData::writeParameterInfo() {
    ...
    fout_params << "Script Name : " << script_name << endl << "Run_condition : " << Run_condition << endl << "Number of Replication : " << Replication << endl << \
     ...
     "mutation_alpha : " << mut_alpha << endl << "mutation_bt : " << mut_bt << endl << \
     "Sim_N : " << SimN << endl << "Task Group_N : " << Taskgroup_N << endl << \
     "type_op : " << "Normal" << endl << \
     "init_alpha : " << init_alpha << endl << "init_inverse_temp : " << init_bt << endl << \
...
void saveData::recordData_trial(Agent agent, int sim_i, int gene_i, int pop_i, int task_i, int trial_i) {
    ...
    double output[13] = {(double)sim_i, (double)gene_i, (double)agent.return_ID(), (double)task_i, (double)trial_i, \
                        agent.return_alpha(), agent.return_bt(), \
...
void saveData::recordData_nat(Agent agent, int sim_i, int gene_i, int pop_i, int task_i) {
    ...
    /*
    // full output version (15 columns)
    double output[15] = {(double)sim_i, (double)gene_i, (double)agent.return_ID(), (double)task_i, (double)agent.return_Offspring(), agent.return_MeanPayoff(task_i), \
    ...
    */
    ...
    // short output version (6 colums)
    double output[6] = {(double)task_i, agent.return_MeanPayoff(task_i),\
                        (double)agent.return_MeanP(task_i), agent.return_rateChoice1(task_i), agent.return_alpha(), agent.return_bt()};
    
    for (int data_i = 0; data_i < 6; ++data_i) {
...
void saveData::recordData_gene(Agent agent, int sim_i, int gene_i, int pop_i) {
    ...
    if (pop_i == PopSize - 1) {
        ...
        */
        ...
        double mean_alpha, mean_bt;
        double sum_alpha = 0.0; double sum_bt = 0.0;

        for (int i = 0; i < PopSize; ++i) {

            //cout << "Name : " << i << " alpha : " << alpha_list[i] << " bt : " << bt_list[i] << endl;
            sum_alpha += alpha_list[i]; 
            sum_bt += bt_list[i];

        }
    
        mean_alpha = sum_alpha/PopSize; mean_bt = sum_bt/PopSize;
        //cout << "mean_alpha: " << mean_alpha << ", mean_bt: " << mean_bt << endl;

        // calculate SD of parameters
        double sd_alpha, sd_bt;
        double var_alpha = 0.0; double var_bt = 0.0;

        for (int i = 0; i < PopSize; ++i) {
            
            var_alpha += (alpha_list[i] - mean_alpha) * (alpha_list[i] - mean_alpha);
            var_bt += (bt_list[i] - mean_bt) * (bt_list[i] - mean_bt);

        }
        
        sd_alpha = sqrt(var_alpha/PopSize); sd_bt = sqrt(var_bt/PopSize);
        //cout << "sd_alpha: " << sd_alpha << ", sd_bt: " << sd_bt << endl;

        double output[6] = {(double)sim_i, (double)gene_i, mean_alpha, mean_bt, sd_alpha, sd_bt};
...
void saveData::writeCSV_nat() {
    ...
    */
    ...
    for (size_t data_i = 0; data_i < (PopSize*Task_N); ++data_i) {
        ...
        for (size_t data_j = 0; data_j < 6; ++data_j) {
...
void saveData::writeCSV_gene() {
    ...
    for (size_t data_i = 0; data_i < 6; ++data_i) {
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    uniform_real_distribution<double> alpha_dist(0.0, 1.0);
    ...
    for (int pop_i = 0; pop_i < PopSize; ++pop_i){
        ...
        if (init_alpha == "R") {
            alpha_i = alpha_dist(get_rand_mt_1);
        }
        else { // "Fixed"
            alpha_i = fixed_alpha;
        }
        ...
        nAgent[pop_i].set_Genes(alpha_i, bt_i);
        //cout << pop_i << ": ";
    ...
    for (int gene_i = 0; gene_i < Generation; ++gene_i) {
        ...
        */
        ...
        //--------------- Copy Genes ---------------//
        // new_alpha[PopSize] .. temp list for storing gene alpha
        //cout << "\nCopy genes.\n" << endl;

        double new_alpha[PopSize];
        double new_bt[PopSize];
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {

            new_alpha[pop_i] = nAgent[vecOffspring[pop_i]].return_alpha();
            new_bt[pop_i] = nAgent[vecOffspring[pop_i]].return_bt();
            //cout << "pop:" << vecOffspring[pop_i] << "; alpha: " << new_alpha[pop_i] << "; bt: " << new_bt[pop_i] << endl;
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            double next_alpha_i;
            double next_bt_i;
            normal_distribution<double> nrml_dist_alpha(new_alpha[pop_i], mut_alpha);
            ...
            if (init_alpha == "R") {
                ...
                do {
                    next_alpha_i = nrml_dist_alpha(get_rand_mt_3);
                    //cout << "next alpha : " << next_alpha_i << endl;
                } while((next_alpha_i < 0)||(next_alpha_i > 1));

            } 
            else { // any fixed value

                next_alpha_i = fixed_alpha;
            ...
            nAgent[pop_i].set_Genes(next_alpha_i, next_bt_i);
        }
