...
res_ql_list_choice <- readRDS(file = here("Data", "SameExp_Sim_RandMultiTask", "res_ql_list_choice.RDS"))

df_agent_randmulti_LastGenePara <- readRDS(here("Data", "Evo_RandMultiTask", "df_agent_randmulti_LastGenePara.obj"))

purrr::map(res_ql_list_choice, 
           ~{.x %>% 
               tibble::as_tibble(.) %>% 
               dplyr::bind_cols(df_agent_randmulti_LastGenePara) %>% 
               tidyr::pivot_longer(
                 cols = starts_with("V"),
                 names_to = "trial",
                 names_prefix = "V",
...
ggsave(plot = g, filename = here(output_path, "Fig1.1_FirstLastGene_RiskAversion_risk-aversion_inrow.png"), width = 20.0, height = 6.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig1.2_FirstLastGene_RiskAversion_risk-seeking_inrow.png"), width = 20.0, height = 6.0)
```
...
df_gene_randmulti_longer %>% 
  ...
  my_theme2 + labs(x = 'Generation', y = 'Parameter', subtitle = "(a)") + guides(color = "none") + 
...
ggsave(plot = g, filename = here(output_path, "Fig5_FirstLastGene_RiskAversion_RandMulti.png"), width = 20.0, height = 5.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig6_Trial_RiskAversion_RandMulti.png"), width = 20.0, height = 8.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig7.1_Heatmap_RiskAversion_risk-aversion.png"), width = 17.0, height = 4.5)
```
...
ggsave(plot = g, filename = here(output_path, "Fig7.2_Heatmap_RiskAversion_risk-seeking.png"), width = 17.0, height = 4.5)
```
...
ggsave(plot = g, filename = here(output_path, "Fig8_RiskPref_RandMulti.png"), width = 18.0, height = 25.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig8_sup_RiskPref_RandMulti_cond2.png"), width = 18.0, height = 6.0)
```
...
df_choice_gain_loss %>% 
  ...
  labs(x = "Gain - loss differece in rate of risk aversion", y = "Frequency of agent") + 
  facet_grid(.~sim_n) -> g

ggsave(plot = g, filename = here(output_path, "Fig9_RiskPrefDif_RandMulti.png"), width = 18.0, height = 6.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig10.1_LastGene_bt_risk-aversion_inrow.png"), width = 20.0, height = 6.0)
```
...
ggsave(plot = g, filename = here(output_path, "Fig10.2_LastGene_bt_risk-seeking_inrow.png"), width = 20.0, height = 6.0)
```
