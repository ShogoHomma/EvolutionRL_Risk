...
ggsave(plot = g, filename = here(output_path, "Fig1.1_FirstLastGene_RiskAversion_risk-aversion_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```
...
g <- VisFirstLastGene_RiskAversion(df_gene_riskaversion_mean, target_simexp, "Risk-seeking task", legend = FALSE)

ggsave(plot = g, filename = here(output_path, "Fig1.2_FirstLastGene_RiskAversion_risk-seeking_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```
...
ggsave(plot = g, filename = here(output_path, "Fig3.1_LastGene_apan_risk-aversion_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```
...
ggsave(plot = g, filename = here(output_path, "Fig3.2_LastGene_apan_risk-seeking_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```
...
df_gene_randmulti_longer %>% 
  ...
  my_theme2 + labs(x = 'Generation', y = 'Parameter value', subtitle = "(a)") + guides(color = "none") + 
...
ggsave(plot = g, filename = here(output_path, "Fig4.1_Evo_RandMulti_apan.png"), width = 19.0, height = 7.5)
```
...
df_gene_randmulti_taskinfo_longer %>% 
  ...
  labs(x = "Mean negative area rate", y = expression(alpha[p]), subtitle = "(b)") -> g

ggsave(plot = g, filename = here(output_path, "Fig4.2_Relation_RandMulti_ap_NegaArea.png"), width = 17.0, height = 6.5)
```


## Fig.5 ランダム複数：各条件でのリスク回避率
...
ggsave(plot = g, filename = here(output_path, "Fig5_FirstLastGene_RiskAversion_RandMulti.png"), width = 18.0, height = 5.0, dpi = 400)
```
...
## Fig.6 ランダム複数：試行を通しての学習ダイナミクスを図示
...
ggsave(plot = g, filename = here(output_path, "Fig6_Trial_RiskAversion_RandMulti.png"), width = 20.0, height = 8.0, dpi = 400)
```



## Fig.7 apとanの組合せと平均リスク回避率
...
ggsave(plot = g, filename = here(output_path, "Fig7.1_Heatmap_RiskAversion_risk-aversion.png"), width = 17.0, height = 4.5, dpi = 400)
```
...
ggsave(plot = g, filename = here(output_path, "Fig7.2_Heatmap_RiskAversion_risk-seeking.png"), width = 17.0, height = 4.5, dpi = 400)
```
...
# apがある値の時にリスク回避率が大きく変わる部分を取り出す
# → やめた
df_rev %>%
  dplyr::mutate(
    sign = case_when(
      mean_risk_aversion > 0.55 ~ '>',
      (mean_risk_aversion > 0.45) & (mean_risk_aversion <= 0.55) ~ '-',
      mean_risk_aversion <= 0.45 ~ '<'
    )
  ) %>% 
  dplyr::select(sim_sort, ap, an, mean_risk_aversion, sign) 
...
ggsave(plot = g, filename = here(output_path, "Fig8_RiskPref_RandMulti_latter100.png"), width = 16.0, height = 24.0, dpi = 400)
```
...
ggsave(plot = g, filename = here(output_path, "Fig8_RiskPref_RandMulti_latter100_cond2.png"), width = 18.0, height = 6.0)
```


## Fig.9 Gain-Lossの差のヒストグラム
...
ggsave(plot = g, filename = here(output_path, "Fig9_RiskPrefDif_RandMulti_latter100_vert.png"), width = 5.0, height = 18.0, dpi = 400)
```
...
- リスク回避増加/減少の割合：gain - loss > 0 (lossに比べてgainでリスク回避傾向が増加した) である個体の数を数えた
...
ggsave(plot = g, filename = here(output_path, "Fig10.1_LastGene_bt_risk-aversion_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```
...
ggsave(plot = g, filename = here(output_path, "Fig10.2_LastGene_bt_risk-seeking_inrow.png"), width = 20.0, height = 6.0, dpi = 400)
```



## Fig.11 複数$\beta$：ランダム複数課題シミュレーションの進化
...
ggsave(plot = g, filename = here(output_path, "Fig11_Evo_RandMulti_bt.png"), width = 17.0, height = 5.0)
```
