...
class Agent{
    ...
    public:
        ...
        double return_rateChoice() {return rateChoice;}
...
void Agent::select_Option(int trial_i) {
    ...
    if (Choice_list[trial_i] == 1) {
        ...
        double payoff = nrml_dist_risk(get_rand_mt_2);
    ...
    // if the agent chose sure option
        ...
        double payoff = nrml_dist_sure(get_rand_mt_2);
...
void Agent::qLearning(int trial_i) {
    ...
    if (delta >= 0) {
        ...
        Qvalue_list[trial_i + 1][Choice_list[trial_i]] = Qvalue_list[trial_i][Choice_list[trial_i]] + ap * delta;
        Qvalue_list[trial_i + 1][1 - Choice_list[trial_i]] = Qvalue_list[trial_i][1 - Choice_list[trial_i]];

    }
    // if PRE is negative
    else if (delta < 0) {

        Qvalue_list[trial_i + 1][Choice_list[trial_i]] = Qvalue_list[trial_i][Choice_list[trial_i]] + an * delta;
        Qvalue_list[trial_i + 1][1 - Choice_list[trial_i]] = Qvalue_list[trial_i][1 - Choice_list[trial_i]];
...
class saveData {
    ...
    vector<vector<double> > output_trial; // 2 dimension list, 17 columns
    vector<double> output_gene; // 1 dimension list, 12 columns
    vector<vector<double> > output_nat; // 2 dimension list, 14 columns
...
saveData::saveData()
{
    ...
    // set gene number to record clocks
    ...
    // initialize output
    output_trial.assign(PopSize*Trial, vector<double>(17, 0.0));
    output_gene.assign(8, 0.0);
    //output_nat.assign(PopSize, vector<double>(14, 0.0));
    output_nat.assign(PopSize, vector<double>(9, 0.0)); // simple output ver.
...
void saveData::create_file() {
    ...
    if (!ofs0) {
        cout << "cannot open file!. Check file_name_params ..." << endl << file_name_trial << endl;
    }
    else {
    ...
    /*
    // for file_trial, we devide into 10 files (cas of large files)
    for (int file_trial_i = 0; file_trial_i < 10; ++file_trial_i) {
        ...
        if (!ofs1) {
            ...
            // 17 columns
            ofs1 << "replication,generation,agent,trial,ap,an,bt,Q1,Q0,p,choice,get_payoff,delta,pay_norm_mean1,pay_norm_sd1,pay_norm_mean2,pay_norm_sd2" << endl;
        }
    ...
    */
    ...
    if (!ofs2) {
        ...
        // 8 columns
        ofs2 << "replication,generation,mean_ap,mean_an,mean_bt,sd_ap,sd_an,sd_bt" << endl;
    }

    ofstream ofs3(file_name_nat);
    if (!ofs3) {
        cout << "cannot open file!. Check file_name_nat ..." << endl << file_name_trial << endl;
    }
    else {
        cout << "succeed! created " << file_name_nat << endl;
        // 14 columns
        //ofs3 << "replication,generation,agent,ancestor,mean_payoff,fitness,rate_choice1,rate0to0,rate0to1,rate1to0,rate1to1,ap,an,bt" << endl;
        ofs3 << "replication,generation,agent,mean_payoff,mean_p,rate_choice1,ap,an,bt" << endl; // short output ver. (9 columns)
    }
...
void saveData::recordData_trial(Agent agent, int replication_i, int gene_i, int pop_i, int trial_i) {
    ...
    double output[17] = {(double)replication_i, (double)gene_i, (double)agent.return_ID(), (double)trial_i, agent.return_ap(), agent.return_an(), agent.return_bt(), agent.return_Q1(trial_i), \
                    ...
                    payoff_norm_mean1, payoff_norm_sigma1, payoff_norm_mean2, payoff_norm_sigma2};
...
void saveData::recordData_nat(Agent agent, int replication_i, int gene_i, int pop_i) {
    ...
    double output[14] = {(double)replication_i, (double)gene_i, (double)agent.return_ID(), (double)agent.return_Offspring(), agent.return_MeanPayoff(), \
                        agent.return_Fitness(), agent.return_rateChoice(), agent.return_rate00(), agent.return_rate01(), agent.return_rate10(), agent.return_rate11(), \
    ...
    */
    ...
    // simple recording version 9 colums
    double output[9] = {(double)replication_i, (double)gene_i, (double)agent.return_ID(), agent.return_MeanPayoff(),\
                        agent.return_MeanP(), agent.return_rateChoice(), agent.return_ap(), agent.return_an(), agent.return_bt()};
...
int main(int argc, char *argv[]){
    ...
    payoff_norm_mean1 = atof(argv[5]);
    payoff_norm_sigma1 = atof(argv[6]);
    payoff_norm_mean2 = atof(argv[7]);
    ...
    for (int gene_i = 0; gene_i < Generation; ++gene_i) {
        ...
        // for each agent: select_option + qlearning
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            for (int trial_i = 0; trial_i < Trial; ++trial_i) {
                ...
                nAgent[pop_i].select_Option(trial_i);
                nAgent[pop_i].qLearning(trial_i);
                //savedat.recordData_trial(nAgent[pop_i], replication_i, gene_i, pop_i, trial_i);
        ...
        // Roulette Wheel Selection (Calculating Fitness)
        ...
        // if minimum payoff is negative, raise the bottom 
        if (min_payoff <= 0) {
        ...
        // creating a multinomial distirbution 
        random_device rd;
        mt19937_64 get_rand_mt_4(rd());
        discrete_distribution<> prob(vecFitness.begin(), vecFitness.end());

        // sampling agents' ID from the multinomial distribution
        vector<int> vecOffspring;

        for (size_t n = 0; n < PopSize; ++n) {
            size_t result = prob(get_rand_mt_4);
        ...
        */
        ...
        for (int pop_i = 0; pop_i < PopSize; ++pop_i) {
            ...
            for (int j = 0; j < PopSize; ++j) {
                ...
                bool_list[j] = vecOffspring[j] == pop_i; // true or not
                // cout << bool_list[j];
        ...
        // new_ap[PopSize] .. temp list for storing genes
        //cout << "\nCopy genes.\n" << endl;
        ...
        */
        ...
        for (int check_i = 0; check_i < 10; ++check_i) {
            ...
            gene_bool = gene_i == savedat.show_gene_time[check_i];
            if (gene_bool == 1) {
